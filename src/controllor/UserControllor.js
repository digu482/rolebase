...
const {passwordencrypt,validatePassword,} = require("../services/CommonServices");
  ...
  try {
    ...
    if (!userLogin) {
      ...
      if (!isvalid) {
        ...
        const { error, token } = await TokenGenerate(userLogin._id, role);
        if (error) {
          return res.status(400).json({
            status: StatusCodes.BAD_REQUEST,
            message: MessageRespons.notcreatetoken,
  ...
  try {
   const token = req.headers.authorization;
    ...
    if (blockTokens.has(token)) {
      ...
      const userfind = await User.findById({ _id: req.decodeduser });
